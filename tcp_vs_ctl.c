 ...
 *
 * Version:     $Id: tcp_vs_ctl.c,v 1.13.2.2 2004/12/08 13:58:13 wensong Exp $
 *
...
tcp_vs_add_rule(struct tcp_vs_service *svc,
		char *pattern, int matchnum, __u32 addr, __u16 port)
{
	tcp_vs_dest_t *dest;
	...
	if (regcomp(&r->rx, pattern, REG_EXTENDED)) {
...
do_tcp_vs_set_ctl(struct sock *sk, int cmd, void *user, unsigned int len)
{
	...
	case TCP_VS_SO_SET_ADDRULE:
		ret = tcp_vs_add_rule(svc, rule->pattern, rule->match_num,
				      rule->addr, rule->port);
		break;
...
tcp_vs_control_start(void)
{
	...
	TCP_VS_ERR("This is tcp_vs_control_start!\n");
	return ret;
}
...
tcp_vs_control_stop(void)
{
	...
	TCP_VS_ERR("This is tcp_vs_control_stop!\n");
}
