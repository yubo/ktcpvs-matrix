 ...
 *
 * Version:	$Id: tcp_vs_http_trans.c,v 1.2 2003/06/14 16:09:47 wensong Exp $
 *
		...
		http_read_ctl_block_t * ctl_blk, int len)
{
	...
	do {
		...
		if (reads == 0) {
			...
			add_wait_queue(ctl_blk->sock->sk->sk_sleep, &wait);
			__set_current_state(TASK_INTERRUPTIBLE);
			schedule();
			__set_current_state(TASK_RUNNING);
			remove_wait_queue(ctl_blk->sock->sk->sk_sleep, &wait);
			continue;
		}
...
http_read_line(http_read_ctl_block_t * ctl_blk, int grow)
{
	...
	while ((nbytes > 0) && (len < 0)) {
		...
		if (ctl_blk->sock->sk->sk_state != TCP_ESTABLISHED
		    && ctl_blk->sock->sk->sk_state != TCP_CLOSE_WAIT) {
			if (len > 0)
				goto done;
			else
				goto exit;
...
data_available(http_read_ctl_block_t * ctl_blk)
{
	...
	if (ctl_blk->remaining == 0) {
		...
		/* Do we have data ? */
		if (skb_queue_empty(&(ctl_blk->sock->sk->sk_receive_queue)))
			ret = 0;
		else
			ret = 1;
		goto out;
